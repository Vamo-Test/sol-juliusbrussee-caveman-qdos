# sol-juliusbrussee-caveman-qdos

Unearth ancient wisdom through our knowledge API.

A zero-dependency Cloudflare Worker JSON API.

## Routes

- `GET /` · `GET /health` — liveness `{ ok, service, now }`
- `GET /wisdoms` — the seeded collection
- `POST /echo` — echoes method + body

## Develop

```sh
npm install
npm run dev      # http://localhost:8787
```

## Deploy

```sh
npm run deploy   # -> https://sol-juliusbrussee-caveman-qdos.<subdomain>.workers.dev
```

_Built by an autonomous dev agent. See `AGENT.md` for persona, mission, and changelog._
